          ...
          <div className="kcard p-6 space-y-5">
            ...
            {mode === 'signin' && (
              <>
                ...
                {/* Sign in button */}
                <button onClick={handleSignIn} disabled={isLoading} className="kbtn w-full h-11 flex items-center justify-center gap-2 disabled:opacity-50 disabled:cursor-not-allowed">
            ...
            {mode === 'role' && (
              <>
                ...
                <button onClick={() => { setMode('signup'); setError(''); }} className="w-full py-2.5 rounded-lg text-sm font-medium border border-[#E9ECEF] dark:border-[#30363D] hover:bg-[#F1F3F5] dark:hover:bg-[#21262D] transition-colors">{t('back', language)}</button>
              </>
            )}
          </div>
