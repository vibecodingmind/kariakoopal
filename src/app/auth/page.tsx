...
function AuthContent() {
  ...
  useEffect(() => {
    if (isAuthenticated) {
      if (showOnboarding) {
        // Onboarding is handled in dashboard page
      }
      router.replace('/dashboard');
    }
  }, [isAuthenticated, router, showOnboarding]);

  // If role was preselected via URL, skip to role step after phone/otp
  // Or if role is already set, go straight to role selection
  useEffect(() => {
    if (preselectedRole && step === 'phone') {
      // Still show phone first, but pre-select the role
    }
  }, [preselectedRole, step]);
