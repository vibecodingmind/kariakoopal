  ...
  return (
    <div className="px-4 py-4 space-y-5">
      <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }}>
        ...
        <p className="text-sm text-[#64748B] mt-1">{purchased}/{items.length} {l('items purchased', 'bidhaa zimenunuliwa')}</p>
      </motion.div>
      ...
      <div className="kcard p-4">
        <div className="flex items-center justify-between mb-2">
          ...
          <span className="text-sm font-bold text-[#065F46]">{Math.round((purchased / items.length) * 100)}%</span>
        </div>
        <div className="kprogress-green">
          <div className="kprogress-bar" style={{ width: `${(purchased / items.length) * 100}%` }} />
        </div>
        <div className="flex items-center justify-between mt-2">
          <span className="text-xs text-[#64748B]">{l('Total', 'Jumla')}: TZS {total.toLocaleString()}</span>
          <span className="text-xs text-[#64748B]">{items.length - purchased} {l('remaining', 'zimesalia')}</span>
        </div>
      </div>
        ...
        <AnimatePresence>
          {items.map(item => (
            <motion.div key={item.id} layout initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, x: -100 }} className={`kcard p-3 flex items-center gap-3 ${item.purchased ? 'opacity-60' : ''}`}>
              <button onClick={() => togglePurchased(item.id)} className={`w-6 h-6 rounded-md border-2 flex items-center justify-center shrink-0 transition-colors ${item.purchased ? 'bg-[#065F46] border-[#065F46]' : 'border-[#E2E8F0] dark:border-[#475569]'}`}>
              ...
              <div className="flex-1 min-w-0">
                ...
                <div className="flex items-center gap-2 mt-0.5">
                  {item.zone && <span className="text-[10px] text-[#64748B] flex items-center gap-0.5"><MapPin className="w-2.5 h-2.5" />{item.zone}</span>}
              ...
              <button onClick={() => removeItem(item.id)} className="text-[#64748B] hover:text-[#DC2626] transition-colors"><Trash2 className="w-4 h-4" /></button>
            </motion.div>
          ))}
        </AnimatePresence>
      </div>
