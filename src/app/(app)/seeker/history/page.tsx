  ...
  return (
    <div className="px-4 py-4 space-y-4">
      <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }}>
        <h1 className="text-2xl font-bold text-[#0A4D3C] dark:text-[#2EA77A]">{l('Session History', 'Historia ya Vipindi')}</h1>
        <p className="text-sm text-[#6C757D] mt-1">{l('Your past guided sessions', 'Vipindi vyako vilivyopita')}</p>
      </motion.div>
          ...
          <motion.div key={session.id} initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.05 }} className="kcard p-4">
            <div className="flex items-center justify-between mb-2">
              <div className="flex items-center gap-2">
                <div className="w-10 h-10 rounded-full bg-[#0B5D3A] flex items-center justify-center text-white text-xs font-bold">{session.guide.split(' ').map(n => n[0]).join('')}</div>
                <div>
                  <p className="font-semibold text-sm">{session.guide}</p>
                  <p className="text-xs text-[#6C757D]">{session.specialty}</p>
                </div>
              </div>
              <div className="text-right">
                <p className="font-bold text-sm text-[#0A4D3C]">TZS {session.amount.toLocaleString()}</p>
                <div className="flex items-center gap-0.5 justify-end">{Array.from({ length: 5 }).map((_, j) => <Star key={j} className={`w-3 h-3 ${j < session.rating ? 'fill-[#FFD23F] text-[#FFD23F]' : 'text-[#E5E7EB]'}`} />)}</div>
              </div>
            </div>
            ...
            <p className="text-xs text-[#6C757D] italic">"{session.review}"</p>
          </motion.div>
        ))}
      </div>
    </div>
