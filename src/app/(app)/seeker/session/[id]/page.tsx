  ...
  return (
    <div className="px-4 py-4 space-y-4">
      <motion.div initial={{ opacity: 0, x: -12 }} animate={{ opacity: 1, x: 0 }}>
        <button onClick={() => router.back()} className="flex items-center gap-1 text-sm text-[#78716C] hover:text-[#0A4D3A]">
      ...
      <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} className="kcard-green p-4">
        ...
        <div className="flex items-center gap-3">
          <div className="w-12 h-12 rounded-full bg-[#D97706] flex items-center justify-center text-[#312E81] font-bold">MJ</div>
          <div>
            <p className="text-white font-bold">Mwanaildi Juma</p>
            <p className="text-white/60 text-xs flex items-center gap-2">
              <span className="flex items-center gap-0.5"><Star className="w-3 h-3 fill-[#D97706] text-[#D97706]" />4.8</span>
      ...
      <div className="kcard p-4">
        <h3 className="font-semibold text-sm mb-3 flex items-center gap-1"><MessageSquare className="w-4 h-4 text-[#3730A3]" />{l('Chat', 'Mazungumzo')}</h3>
      ...
      <div className="grid grid-cols-3 gap-2">
        <button className="kcard p-3 text-center"><Phone className="w-5 h-5 text-[#3730A3] mx-auto mb-1" /><span className="text-[10px] font-medium">{l('Call', 'Piga')}</span></button>
