  ...
  return (
    <div className="px-4 py-4 space-y-5">
      <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }}>
        <h1 className="text-2xl font-bold text-[#065F46] dark:text-[#34D399]">{l('Market Stories', 'Hadithi za Soko')}</h1>
        <p className="text-sm text-[#64748B] mt-1">{l('Local insights, tips & tales from Kariakoo', 'Maoni ya karibu, mapendekezo na hadithi kutoka Kariakoo')}</p>
      </motion.div>
      ...
      {featured && (
        <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} className="kcard-green p-5 cursor-pointer" onClick={() => setExpandedId(expandedId === featured.id ? null : featured.id)}>
          <div className="flex items-center gap-2 mb-2">
            ...
            {featured.hasAudio && <Volume2 className="w-3.5 h-3.5 text-[#F59E0B]" />}
          </div>
        ...
        {filtered.filter(s => !s.featured).map((story, i) => (
          ...
          >
            <div className="flex items-start gap-3">
              <div className="w-10 h-10 rounded-lg bg-[#ECFDF5] dark:bg-[#022C22] flex items-center justify-center shrink-0">
                <BookOpen className="w-5 h-5 text-[#065F46] dark:text-[#34D399]" />
              </div>
              <div className="flex-1">
                <div className="flex items-center gap-2 mb-0.5">
                  {story.tags.map(tag => <span key={tag} className="kbadge kbadge-verified text-[8px]">{tag}</span>)}
                  {story.hasAudio && <Volume2 className="w-3 h-3 text-[#065F46]" />}
                </div>
            ...
            <p className="text-sm text-[#64748B] mt-2 leading-relaxed">{sw ? story.excerptSw : story.excerpt}</p>
            <AnimatePresence>
              {expandedId === story.id && (
                <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }} exit={{ opacity: 0, height: 0 }} className="overflow-hidden">
                  <p className="text-sm text-[#64748B] mt-2 leading-relaxed whitespace-pre-line">{sw ? story.contentSw : story.content}</p>
                </motion.div>
              )}
            </AnimatePresence>
