  ...
  return (
    <div className="px-4 py-4 space-y-5">
      <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }}>
        <h1 className="text-2xl font-bold text-[#0A4D3C] dark:text-[#2EA77A]">{l('Settings', 'Mipangilio')}</h1>
        <p className="text-sm text-[#6C757D] mt-1">{l('Manage your account and preferences', 'Simamia akaunti yako na mapendeleo')}</p>
      </motion.div>

      {/* Account Section */}
      <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }}>
        <div className="flex items-center justify-between mb-3">
          <h2 className="text-sm font-bold text-[#6C757D] uppercase tracking-wider">{l('Account', 'Akaunti')}</h2>
        ...
        <div className="kcard p-0 overflow-hidden">
          <div className="p-5 border-b border-[#E9ECEF] dark:border-[#30363D]">
            <div className="flex items-center gap-4">
              <div className="relative">
                <div className="w-16 h-16 rounded-2xl bg-gradient-to-br from-[#0A4D3C] to-[#2EA77A] flex items-center justify-center text-white font-bold text-xl">
                  {user?.name?.charAt(0) || 'U'}
                </div>
                {editing && (
                  <button className="absolute -bottom-1 -right-1 w-7 h-7 rounded-full bg-[#FFD23F] flex items-center justify-center shadow-md">
          ...
          <div className="divide-y divide-[#E9ECEF] dark:divide-[#30363D]">
            <div className="px-4 py-3 flex items-center justify-between">
              <div className="flex items-center gap-3">
                <User className="w-4 h-4 text-[#0B5D3A]" />
              ...
              {editing ? (
                ...
                <span className="text-sm text-[#6C757D]">{user?.name}</span>
              )}
            </div>
            <div className="px-4 py-3 flex items-center justify-between">
              <div className="flex items-center gap-3">
              ...
              {editing ? (
                ...
                <span className="text-sm text-[#6C757D]">{user?.phone}</span>
              )}
            </div>
            <div className="px-4 py-3 flex items-center justify-between">
              <div className="flex items-center gap-3">
                <Mail className="w-4 h-4 text-[#0B5D3A]" />
              ...
              {editing ? (
                ...
                <span className="text-sm text-[#6C757D]">{user?.email || l('Not set', 'Haijawekwa')}</span>
              )}
            </div>
          </div>
        </div>
      ...
      <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.15 }}>
        <h2 className="text-sm font-bold text-[#6C757D] uppercase tracking-wider mb-3">{l('Preferences', 'Mapendeleo')}</h2>
        <div className="kcard p-0 overflow-hidden">
          <div className="px-4 py-3.5 flex items-center justify-between">
            <div className="flex items-center gap-3">
              <Globe className="w-4 h-4 text-[#0B5D3A]" />
              <span className="text-sm font-medium">{l('Language', 'Lugha')}</span>
            </div>
            <div className="flex gap-1">
              <button
                onClick={() => setLanguage('sw')}
                className={`px-3 py-1.5 rounded-lg text-xs font-semibold transition-colors ${language === 'sw' ? 'bg-[#0A4D3C] text-white' : 'bg-[#F1F3F5] dark:bg-[#21262D] text-[#6C757D]'}`}
              >
                Kiswahili
              </button>
              <button
                onClick={() => setLanguage('en')}
                className={`px-3 py-1.5 rounded-lg text-xs font-semibold transition-colors ${language === 'en' ? 'bg-[#0A4D3C] text-white' : 'bg-[#F1F3F5] dark:bg-[#21262D] text-[#6C757D]'}`}
              >
                English
              </button>
            </div>
          ...
          <div className="border-t border-[#E9ECEF] dark:border-[#30363D]">
            <div className="px-4 py-3.5 flex items-center justify-between">
              <div className="flex items-center gap-3">
                <Bell className="w-4 h-4 text-[#0B5D3A]" />
                <div>
                  <span className="text-sm font-medium">{l('Push Notifications', 'Arifa za Simu')}</span>
                  <p className="text-[10px] text-[#6C757D]">{l('Session alerts, payments', 'Arifa za vipindi, malipo')}</p>
                </div>
              </div>
              <button onClick={() => setPushNotif(!pushNotif)}>
                <div className={`w-10 h-5.5 rounded-full relative transition-colors ${pushNotif ? 'bg-[#0A4D3C]' : 'bg-[#E9ECEF] dark:bg-[#30363D]'}`}>
            ...
            <div className="px-4 py-3.5 flex items-center justify-between">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-[10px] text-[#6C757D]">{l('Updates, promotions', 'Taarifa, matangazo')}</p>
                </div>
              </div>
              <button onClick={() => setEmailNotif(!emailNotif)}>
                <div className={`w-10 h-5.5 rounded-full relative transition-colors ${emailNotif ? 'bg-[#0A4D3C]' : 'bg-[#E9ECEF] dark:bg-[#30363D]'}`}>
          ...
          <div className="border-t border-[#E9ECEF] dark:border-[#30363D]">
            <div className="px-4 py-3.5 flex items-center justify-between">
              <div className="flex items-center gap-3">
                <Eye className="w-4 h-4 text-[#0B5D3A]" />
                <div>
                  <span className="text-sm font-medium">{l('Profile Visibility', 'Mwonekano wa Wasifu')}</span>
                  <p className="text-[10px] text-[#6C757D]">{l('Visible to other users', 'Inaonekana kwa wengine')}</p>
                </div>
              </div>
              <button onClick={() => setProfileVisible(!profileVisible)}>
                <div className={`w-10 h-5.5 rounded-full relative transition-colors ${profileVisible ? 'bg-[#0A4D3C]' : 'bg-[#E9ECEF] dark:bg-[#30363D]'}`}>
      ...
      <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }}>
        <h2 className="text-sm font-bold text-[#6C757D] uppercase tracking-wider mb-3">{l('More', 'Zaidi')}</h2>
        <div className="kcard p-0 overflow-hidden">
          {[
            ...
            <button
              ...
              className="w-full flex items-center justify-between px-4 py-3.5 hover:bg-[#F1F3F5] dark:hover:bg-[#21262D] transition-colors border-b border-[#E9ECEF] dark:border-[#30363D] last:border-0"
            >
              <div className="flex items-center gap-3">
      ...
      <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.25 }}>
        <button
          ...
          className="w-full py-3 rounded-xl border-2 border-[#E63946] text-[#E63946] font-semibold text-sm hover:bg-[#FEE2E2] dark:hover:bg-[#3D1F1F] transition-colors flex items-center justify-center gap-2 active:scale-95"
        >
      ...
      <div className="text-center py-4">
        <p className="text-[10px] text-[#6C757D]">Kariako Guide v2.0 · {l('Made with love in Kariakoo', 'Imetengenezwa kwa upendo Kariakoo')}</p>
      </div>
    </div>
  );
}
