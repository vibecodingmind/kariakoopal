  ...
  return (
    <div className="px-4 py-4 space-y-5">
      <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }}>
        <h1 className="text-2xl font-bold text-[#0A4D3C] dark:text-[#2EA77A]">{l('Admin Dashboard', 'Dashibodi ya Msimamizi')}</h1>
        <p className="text-sm text-[#6C757D] mt-1">{l('Platform overview & management', 'Muhtasari wa jukwaa na usimamizi')}</p>
      </motion.div>

      {/* Stats Grid */}
      <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }} className="grid grid-cols-2 gap-3">
        {[
          { icon: Users, label: l('Total Users', 'Watumiaji'), value: '12,450', color: '#0B5D3A' },
          { icon: Compass, label: l('Active Guides', 'Miongozo Hai'), value: '2,500', color: '#2EA77A' },
          { icon: Store, label: l('Vendors', 'Wauzaji'), value: '3,200', color: '#FFA500' },
          { icon: DollarSign, label: l('Revenue (MTZS)', 'Mapato (MTZS)'), value: '450', color: '#FFD23F' },
        ].map((stat, i) => (
          <div key={i} className="kcard p-4">
            <stat.icon className="w-5 h-5 mb-2" style={{ color: stat.color }} />
            <p className="text-2xl font-bold">{stat.value}</p>
            <p className="text-[10px] text-[#6C757D]">{stat.label}</p>
          </div>
        ))}
      </motion.div>
      ...
      <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.15 }} className="kcard p-4">
        <div className="flex items-center justify-between mb-3">
          <h3 className="font-semibold text-sm flex items-center gap-1"><BarChart3 className="w-4 h-4 text-[#0B5D3A]" />{l('Revenue Trend', 'Mwelekeo wa Mapato')}</h3>
        ...
        <div className="h-32 flex items-end gap-1.5">
          {[40, 65, 55, 80, 70, 90, 85, 95, 75, 88, 92, 100].map((h, i) => (
            <div key={i} className="flex-1 rounded-t-sm bg-[#0B5D3A] transition-all hover:bg-[#2EA77A]" style={{ height: `${h}%` }} />
          ))}
        </div>
        <div className="flex justify-between mt-1 text-[9px] text-[#6C757D]">
        ...
        <div className="grid grid-cols-2 gap-3">
          {[
            ...
            <button key={i} onClick={() => router.push(action.href)} className="kcard p-4 text-left hover:shadow-md transition-all">
              <action.icon className="w-6 h-6 text-[#0B5D3A] mb-2" />
              <h4 className="font-semibold text-xs">{action.label}</h4>
              <p className="text-[10px] text-[#6C757D] mt-0.5">{action.count}</p>
            </button>
          ))}
        </div>
      </motion.div>
        ...
        <div className="space-y-2">
          {[
            ...
            <div key={i} className="kcard p-3 flex items-start gap-2">
              <div className={`w-2 h-2 rounded-full mt-1.5 shrink-0 ${activity.type === 'guide' ? 'bg-[#0B5D3A]' : activity.type === 'dispute' ? 'bg-[#E63946]' : activity.type === 'fraud' ? 'bg-[#F59E0B]' : 'bg-[#0077B6]'}`} />
              <div>
                <p className="text-xs">{activity.text}</p>
                <p className="text-[10px] text-[#6C757D] mt-0.5">{activity.time}</p>
              </div>
            </div>
          ))}
        </div>
