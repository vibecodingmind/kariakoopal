  ...
  return (
    <div className="px-4 py-4 space-y-5">
      <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }}>
        <h1 className="text-2xl font-bold text-[#0A4D3C] dark:text-[#2EA77A]">{l('Manage Vendors', 'Simamia Wauzaji')}</h1>
      </motion.div>
          ...
          {PENDING_VENDORS.map((vendor, i) => (
            <motion.div key={vendor.id} initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.05 }} className="kcard p-4 border-l-4 border-[#F59E0B]">
              <div className="flex items-center gap-3 mb-2">
                ...
                <div><h4 className="font-semibold text-sm">{vendor.name}</h4><p className="text-xs text-[#6C757D]">{vendor.zone} · {l('Stall', 'Duka')} {vendor.stall} · {vendor.submitted}</p></div>
              </div>
      ...
      <div>
        <h2 className="text-sm font-bold mb-2 flex items-center gap-1"><ShieldCheck className="w-4 h-4 text-[#0B5D3A]" />{l('Verified Vendors', 'Wauzaji Waliothibitishwa')} ({VERIFIED_VENDORS.length})</h2>
        <div className="space-y-2">
          {VERIFIED_VENDORS.map((vendor, i) => (
            <div key={vendor.id} className="kcard p-3 flex items-center justify-between">
              <div className="flex items-center gap-2">
                <div className="w-8 h-8 rounded-full bg-[#E8F5EE] flex items-center justify-center text-[#0B5D3A] font-bold text-xs">{vendor.name.charAt(0)}</div>
                <div><p className="text-sm font-medium">{vendor.name}</p><p className="text-xs text-[#6C757D]">{vendor.zone} · {vendor.stall} · {l('Expires', 'Inaisha')} {vendor.expires}</p></div>
              </div>
              <QrCode className="w-4 h-4 text-[#0B5D3A]" />
            </div>
          ))}
        </div>
      </div>
