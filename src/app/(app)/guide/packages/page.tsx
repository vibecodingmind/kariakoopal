    ...
    <div className="px-4 py-4 space-y-5">
      <div className="flex items-center justify-between">
        <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }}>
          <h1 className="text-2xl font-bold text-[#0A4D3C] dark:text-[#2EA77A]">{l('Package Deals', 'Pakiti za Biashara')}</h1>
          <p className="text-sm text-[#6C757D] mt-1">{l('Create curated tour packages for seekers', 'Unda pakiti za ziara kwa watafutaji')}</p>
        </motion.div>
        ...
        {PACKAGES.map((pkg, i) => (
          <motion.div key={pkg.id} initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.05 }} className="kcard p-4">
            ...
            <div className="flex gap-2">
              ...
              <button className="text-xs py-1.5 px-3 border border-[#E63946] text-[#E63946] rounded-lg flex items-center gap-1"><Trash2 className="w-3 h-3" />{l('Delete', 'Futa')}</button>
            </div>
          </motion.div>
        ))}
      </div>
