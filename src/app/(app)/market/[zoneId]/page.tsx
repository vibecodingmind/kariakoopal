...
const ZONE_DATA: Record<string, {
  ...
  electronics: {
    name: 'Electronics Zone', nameSw: 'Eneo la Elektroniki', color: '#0077B6',
  ...
  fabrics: {
    name: 'Fabrics Zone', nameSw: 'Eneo la Vitenge', color: '#8A2BE2',
  ...
  kitchenware: {
    name: 'Kitchenware Zone', nameSw: 'Eneo la Chombo', color: '#FFA500',
  ...
  if (!zone) {
    return (
      <div className="px-4 py-8 text-center">
        <p className="text-lg font-semibold text-[#6C757D]">{l('Zone not found', 'Eneo halijapatikana')}</p>
  ...
  return (
    <div className="px-4 py-4 space-y-5">
      ...
      <motion.div initial={{ opacity: 0, x: -12 }} animate={{ opacity: 1, x: 0 }}>
        <button onClick={() => router.push('/market')} className="flex items-center gap-1 text-sm text-[#6C757D] hover:text-[#0A4D3A] transition-colors">
      ...
      <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} className="kcard p-5" style={{ borderLeft: `4px solid ${zone.color}` }}>
        <div className="flex items-center gap-3 mb-3">
          ...
          <div>
            ...
            <p className="text-xs text-[#6C757D]">{zone.stalls}+ {l('stalls', 'maduka')}</p>
          </div>
        </div>
        <p className="text-sm text-[#6C757D]">{sw ? zone.descSw : zone.desc}</p>
        <div className="mt-3 p-3 rounded-lg bg-[#FFD23F]/10 border border-[#FFD23F]/20">
          <p className="text-xs font-medium text-[#0A4D3A] dark:text-[#2EA77A]">💡 {l('Insider Tip', 'Ushauri wa Ndani')}: {sw ? zone.tipsSw : zone.tips}</p>
        </div>
      </motion.div>
      ...
      <div className="ksearch flex items-center gap-2 px-4 py-3">
        ...
        />
        <button onClick={() => setViewMode(viewMode === 'grid' ? 'list' : 'grid')} className="text-[#6C757D]">
      ...
      {/* Vendors */}
      <div>
        <h2 className="text-lg font-bold text-[#333] dark:text-[#F0F6FC] mb-3">{l('Vendors', 'Wauzaji')} ({filteredVendors.length})</h2>
          ...
          {filteredVendors.map((vendor, i) => (
            ...
            >
              <div className="flex items-center gap-3">
                <div className="w-10 h-10 rounded-full bg-[#0B5D3A] flex items-center justify-center text-white font-bold text-sm shrink-0">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-xs text-[#6C757D]">{l('Stall', 'Duka')} {vendor.stall}</p>
                </div>
              </div>
              <div className="flex items-center gap-3 mt-3">
                ...
                <div className="flex items-center gap-1">
                  <ThumbsUp className="w-3 h-3 text-[#0B5D3A]" />
      ...
      {zone.prices.length > 0 && (
        <div>
          <h2 className="text-lg font-bold text-[#333] dark:text-[#F0F6FC] mb-3">{l('Price Range', 'Kiwango cha Bei')}</h2>
          <div className="space-y-2">
            {zone.prices.map((p, i) => (
              <div key={i} className="kcard p-3 flex items-center justify-between">
                <div>
                  <p className="text-sm font-medium">{p.item}</p>
                  <p className="text-xs text-[#6C757D]">TZS {p.min.toLocaleString()} - {p.max.toLocaleString()}</p>
                </div>
