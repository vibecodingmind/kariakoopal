  ...
  return (
    <div className="px-4 py-4 space-y-5">
      ...
      <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.4 }}>
        <h1 className="text-2xl font-bold text-[#065F46] dark:text-[#34D399]">{l('Explore Market', 'Gundua Soko')}</h1>
      ...
      <div className="flex items-center justify-between">
        <h2 className="text-lg font-bold text-[#0F172A] dark:text-[#E2E8F0]">{l('Zones', 'Maeneo')}</h2>
        <button onClick={() => setViewMode(viewMode === 'grid' ? 'list' : 'grid')} className="text-[#64748B] hover:text-[#065F46] transition-colors">
      ...
      <div className={viewMode === 'grid' ? 'grid grid-cols-2 gap-3' : 'space-y-3'}>
        {filteredZones.map((zone, i) => (
          ...
          >
            ...
            <h3 className="font-semibold text-sm text-[#0F172A] dark:text-[#E2E8F0] group-hover:text-[#065F46] dark:group-hover:text-[#34D399] transition-colors">
            ...
            <div className="flex items-center justify-between mt-3">
              ...
              <ChevronRight className="w-4 h-4 text-[#64748B] group-hover:text-[#065F46] transition-colors" />
            </div>
          </motion.div>
        ))}
      </div>
      ...
      <div>
        <div className="flex items-center justify-between mb-3">
          <h2 className="text-lg font-bold text-[#0F172A] dark:text-[#E2E8F0]">{l('Featured Vendors', 'Wauzaji Bora')}</h2>
          <button onClick={() => router.push('/vendors')} className="text-sm font-medium text-[#0A4D3A] dark:text-[#34D399]">{l('See All', 'Tazama Zote')}</button>
        </div>
          ...
          {FEATURED_VENDORS.map((vendor, i) => (
            ...
            >
              <div className="flex items-center gap-2 mb-2">
                <div className="w-8 h-8 rounded-full bg-[#065F46] flex items-center justify-center text-white text-xs font-bold">
                  {vendor.name.charAt(0)}
                </div>
                {vendor.verified && <ShieldCheck className="w-4 h-4 text-[#065F46]" />}
              </div>
              <h4 className="font-semibold text-sm truncate">{vendor.name}</h4>
              <p className="text-xs text-[#64748B] mt-0.5">{vendor.stall}</p>
      ...
      <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.4 }} className="kcard-green p-5 text-center">
        ...
        <div className="grid grid-cols-3 gap-4 mt-4">
          <div>
            <p className="text-2xl font-bold text-[#F59E0B]">10K+</p>
          ...
          <div>
            <p className="text-2xl font-bold text-[#F59E0B]">2.5K+</p>
