...
type FilterType = 'all' | 'guide' | 'vendor' | 'zone' | 'food' | 'shopping' | 'experience';

// ── Trending & Recent Searches ──
const TRENDING_SEARCHES = ['Fabrics', 'Spices', 'Electronics', 'Kanga', 'Cultural Tour', 'Wholesale'];
const RECENT_SEARCHES_DEFAULT = ['Fabric guide', 'Electronics zone', 'Spice market'];
...
export default function SearchPage() {
  ...
  useEffect(() => {
    ...
    const allTerms = [
      ...
      'Mwanamvua Juma', 'Asha Mohamed', 'Fatma Hassan',
      'Central Market', 'Fabrics Zone', 'Spice Market',
      'Wholesale', 'Textiles', 'Herbs', 'Gadgets',
    ];
  ...
  }, [query, activeFilter, priceMin, priceMax, minRating, selectedZones, sortBy]);
      ...
      <div className="bg-gradient-to-b from-[#065F46] to-[#064E3B] dark:from-[#0F172A] dark:to-[#0F172A] px-4 pt-4 pb-6">
        <motion.div initial={{ opacity: 0, y: -12 }} animate={{ opacity: 1, y: 0 }}>
          ...
          <div className="relative">
            <div className="flex items-center gap-2 bg-white dark:bg-[#1E293B] rounded-2xl px-4 py-3 shadow-lg">
              ...
              <input
                ...
                placeholder={l('Search anything...', 'Tafuta chochote...')}
              ...
              <button className="p-1.5 hover:bg-[#F1F5F9] dark:hover:bg-[#334155] rounded-full transition-colors">
                <Mic className="w-4 h-4 text-[#065F46] dark:text-[#34D399]" />
              </button>
              <button
            ...
            <div className="px-4 py-4 bg-white dark:bg-[#1E293B] border-b border-[#E2E8F0] dark:border-[#334155]">
              ...
              <div className="mb-4">
                ...
                <input
                  ...
                  className="w-full mt-2 accent-[#065F46]"
                />
                <div className="flex justify-between text-[10px] text-[#64748B] mt-1">
                  <span>TZS 0</span>
                  <span>TZS {priceMax.toLocaleString()}</span>
                </div>
              </div>
              ...
              <div className="mb-4">
                ...
                <div className="flex gap-1.5">
                  ...
                  <span className="flex items-center text-xs text-[#64748B] ml-2">
                    {minRating > 0 ? `${minRating}+ ${l('stars', 'nyota')}` : l('Any', 'Yoyote')}
                  </span>
                </div>
              </div>
                  ...
                  {ZONES.map(zone => (
                    ...
                    >
                      <div className={`w-3 h-3 rounded border-2 flex items-center justify-center ${
                        selectedZones.includes(zone.id) ? 'border-white bg-white' : 'border-[#CBD5E1]'
                      }`}>
                        {selectedZones.includes(zone.id) && <div className="w-1.5 h-1.5 rounded-sm bg-[#065F46]" />}
                      </div>
              ...
              <div className="mb-4">
                <label className="text-xs font-medium text-[#64748B] mb-2 block flex items-center gap-1.5">
                  <Clock className="w-3 h-3" /> {l('Availability', 'Upatikanaji')}
                </label>
                <div className="flex gap-2">
                  {AVAILABILITY_OPTIONS.map(opt => (
                    <button
                      key={opt.id}
                      onClick={() => setAvailability(availability === opt.id ? '' : opt.id)}
                      ...
                      <div className="flex items-start gap-3">
                        ...
                        <div className="flex-1 min-w-0">
                          <div className="flex items-center gap-2 mb-0.5">
                            <h4 className="font-semibold text-sm text-[#065F46] dark:text-white truncate">{result.name}</h4>
                          ...
                          <div className="flex items-center gap-3 flex-wrap">
                            ...
                            {result.specialty && (
                              <span className="text-xs text-[#64748B]">{result.specialty}</span>
                            )}
                            {result.category && (
              ...
              >
                ...
                <p className="text-sm text-[#64748B] max-w-xs mb-6">
                  {l(
                    ...
                    `Hatukupata chochote kinacholingana na "${query}". Jaribu utafutaji tofauti au rekebisha vichujio vyako.`
                  )}
                </p>
