...
const ZONE_COLORS: Record<string, string> = {
  ...
  spices: '#EF4444', kitchenware: '#FFA500', artisanal: '#8B5E3C', all: '#0B5D3A',
};
  ...
  return (
    <div className="px-4 py-4 space-y-5">
      <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }}>
        <h1 className="text-2xl font-bold text-[#0A4D3C] dark:text-[#2EA77A]">{l('Browse Guides', 'Vinjua Miongozo')}</h1>
        <p className="text-sm text-[#6C757D] mt-1">{l('Verified local experts who know Kariakoo inside out', 'Wataalamu wa karibu waliothibitishwa wanaojua Kariakoo vizuri')}</p>
      </motion.div>
      ...
      <div className="ksearch flex items-center gap-2 px-4 py-3">
        ...
        <button onClick={() => setSortBy(sortBy === 'rating' ? 'sessions' : 'rating')} className="text-xs text-[#6C757D] hover:text-[#0A4D3A] font-medium">
          ...
          >
            <div className="flex items-center gap-3">
              <div className="relative">
                <div className={`w-12 h-12 rounded-full flex items-center justify-center text-white font-bold ${guide.status === 'online' ? 'ring-2 ring-[#10B981]' : guide.status === 'busy' ? 'ring-2 ring-[#F59E0B]' : ''}`} style={{ background: ZONE_COLORS[guide.zone] || '#0B5D3A' }}>
                  {guide.name.split(' ').map(n => n[0]).join('')}
                </div>
                <div className="absolute -bottom-0.5 -right-0.5 w-4 h-4 rounded-full border-2 border-white dark:border-[#161B22]" style={{ background: STATUS_MAP[guide.status].color }} />
              </div>
              <div className="flex-1 min-w-0">
                ...
                <div className="flex items-center gap-3 mt-1">
                  <div className="flex items-center gap-1"><Star className="w-3 h-3 fill-[#FFD23F] text-[#FFD23F]" /><span className="text-xs font-medium">{guide.rating}</span></div>
                  <div className="flex items-center gap-1"><Users className="w-3 h-3 text-[#6C757D]" /><span className="text-xs text-[#6C757D]">{guide.sessions} {l('sessions', 'vipindi')}</span></div>
